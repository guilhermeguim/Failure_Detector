...
function update_values(Lclass_value) {
  ...
  $.getJSON($SCRIPT_ROOT + '/newdata', { class_value: Lclass_value }, function(data) {
    ...
    table += '<table class="table table-dark table-striped table-hover table-bordered">';
      table += '<thead id="t_head">';
        table += '<tr>';
          table += '<th scope="col" style="width: 12%;" class="text-center">ID</th>';
          table += '<th scope="col" style="width: 8%;" class="text-center">D_TYPE</th>';
          table += '<th scope="col" style="width: 10%;" class="text-center">ADDRESS</th>';
          table += '<th scope="col" style="width: 8%;" class="text-center">CLASS</th>';
          table += '<th scope="col" class="text-center">DESCRIPTION</th>';
          table += '<th scope="col" style="width: 20%;" class="text-center">DATETIME</th>';
        table += '</tr>';
      table += '</thead>';
      table += '<tbody class="table-group-divider" id="table-body">';

    // Determine how many rows are needed to display the data
    var rows_needed = 12; // assuming each row is 40px high
